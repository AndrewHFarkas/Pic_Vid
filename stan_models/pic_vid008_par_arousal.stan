...
model {
  ...
  for (i in 1:nobs){
    if (type[i] == 1){
      lpp_mu[counter_lpp] = bint_lpp + baro_lpp*arousal[i];
      counter_lpp += 1;
    }
    if (type[i] == 2){
      ssvep_mu[counter_ssvep] = bint_ssvep + baro_ssvep*arousal[i];
      counter_ssvep += 1;
    }
  }
...
generated quantities {
  ...
  for (i in 1:nobs){
    if (type[i] == 1){
      real lpp_mu_gq = bint_lpp + baro_lpp*arousal[i];
      log_lik[i] = normal_lpdf(amp[i] | lpp_mu_gq, lpp_sd);
  //     lpp_pred[i] = normal_rng(lpp_mu_gq, lpp_sd);
    }
    if (type[i] == 2){
      real ssvep_mu_gq = bint_ssvep + baro_ssvep*arousal[i];
