...
generated quantities {
  ...
  vector[nobs] log_lik;  // Log likelihood for amplitude predictions
  
  for (i in 1:nobs) {
    // Decompose the covariance matrix to get the conditional parameters
    real mu_amp_given_aro = par_mu_amp[par[i]] + 
    ...
    log_lik[i] = normal_lpdf(amp[i] | mu_amp_given_aro, sqrt(var_amp_given_aro));
  }
}
