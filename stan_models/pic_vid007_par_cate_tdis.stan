...
model {
  ...
  pic_cate_sd ~ exponential(0.01);
  pic_par_df ~ gamma(100,2);
  pic_cate_df ~ gamma(6,2);
  ...
  for (i in 1:nobs){
    if (type[i] == 1){
      lpp_mu[counter_lpp] = bpar_lpp[par[i]] + bcate_lpp[cate[i]];
      counter_lpp += 1;
    }
    if (type[i] == 2){
      ssvep_mu[counter_ssvep] = bpar_ssvep[par[i]] + bcate_ssvep[cate[i]];
      counter_ssvep += 1;
    }
  }
...
generated quantities {
  ...
  for (i in 1:nobs){
    if (type[i] == 1){
      real lpp_mu_gq = bpar_lpp[par[i]] + bcate_lpp[cate[i]];
      log_lik[i] = normal_lpdf(amp[i] | lpp_mu_gq, lpp_sd);
  //     lpp_pred[i] = normal_rng(lpp_mu_gq, lpp_sd);
    }
    if (type[i] == 2){
      real ssvep_mu_gq = bpar_ssvep[par[i]] + bcate_ssvep[cate[i]];
